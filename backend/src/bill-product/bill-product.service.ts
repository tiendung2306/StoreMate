...
export class BillProductService {
  ...
  async addProductToBill(
    bill_id: number,
    product_id: number,
    quantity: number,
  ) {
    const billProduct = await this.prisma.billProduct.findFirst({
      where: { bill_id, product_id },
    });
    if (!billProduct) {
      const newBillProduct = await this.createBillProduct({
        bill_id,
        product_id,
        quantity,
      });
  ...
  async removeProductFromBill(
    ...
    if (billProduct.quantity <= 0) {
      return this.prisma.billProduct.deleteMany({
        where: { bill_id, product_id },
      });
    }
    return this.prisma.billProduct.update({
      where: { id: billProduct.id },
      data: { quantity: billProduct.quantity },
    });
  }
}
